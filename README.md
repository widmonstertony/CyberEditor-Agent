...
完整流程不是把所有视频简单首尾相接。每个素材会被依次转写，并按场景变化与 5–8 秒
时间证据抽取真实画面，使视觉模型理解动作的开始、发展、反应和结果，而不是只识别物体。
AI 先根据全部素材生成包含主题、四段叙事节拍、目标时长、可执行调色圣经和音乐情绪的
导演阐述，再让视觉模型对长素材按最多 3 分钟的微场景窗口结合画面和台词挑选候选片段。
...
4. **最终导演**：Qwen3.6 再次加载，以最多 3 分钟的视觉微场景窗口结合画面与台词审片并落盘
